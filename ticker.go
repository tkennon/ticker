...
// The jitter will be within `fraction` of the current ticker. e.g. if
// WithJitter(0.2) is applied to an exponential ticker that would otherwise fire
// on 1, 2, 4, 8, ... seconds then the first ticker will fire in between 0.8-1.2
// seconds, and the second will fire between 1.6-2.4 seconds. The jitter
// fraction may be greater than one, allowing the possiblity for jittered
...
// WithMaxDuration sets the maxiumum cumulative total duration the ticker will
// run for. Once the maximum duration is reached, the ticker's channel will be
// closed.
...
// the tickers channel will be closed.
...
// allows interval is greater than the maximum allowed interval). The ticker's
